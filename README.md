<div align="center">

# **Mason's Arcade**

### A collection of 90+ browser-based games — no frameworks, no dependencies, just pure HTML/CSS/JS.

[**Play Now**](https://masonp25.github.io/calculator/game-site/)

---

</div>

## What's Inside

- **90+ Games** across solo, local multiplayer, and online multiplayer
- **Online Multiplayer** with real-time WebSocket rooms and chat
- **Global Leaderboards** powered by Firebase
- **User Accounts** to track scores, coins, and progress across devices
- **Coin Economy** — earn coins from leaderboard placements, daily challenges, and streaks
- **Avatar System** with 60+ items including hats, hair, faces, and shirts
- **Friends System** with friend requests, online status, and activity tracking
- **Daily Challenges & Weekly Quests** with bonus rewards
- **Leveling & XP** system with rank progression
- **Badges & Achievements** for milestones
- **Item Trading & Gifting** between players
- **Game Ratings** — rate and review games
- **Game Invites** — invite friends to play
- **Live Activity Feed** showing leaderboard records and achievements
- **Notifications** for friend requests, score changes, and more
- **Daily Login Streaks** with coin rewards
- **25 Themes** that apply across every game
- **Sound Effects** with mute toggle
- **Animated Page Transitions**

## Games

**Classics** — Tetris, Breakout, Asteroids, Space Invaders, Pac-Man, Galaga, Minesweeper, Solitaire, 2048, Flappy Bird, Snake, Dino Runner

**Puzzle** — Sudoku, Nonogram, Wordle, Connections, Crossword, Flood Fill, Lights Out, Pipe Puzzle, Ball Sort, Hue Sort, Sliding Puzzle, Match-3, Block Blast, Word Search, Wordscapes, 20 Questions

**Action** — Fruit Slicer, Geometry Dash, Doodle Jump, Crossy Road, Aim Trainer, Bullet Dodge, Stickman Hook, Helicopter, Color Switch, Platformer, Paper.io, Gravity Run

**Word** — Typing Test, Hangman, Word Scramble, Wordscapes, Crossword

**Strategy** — Chess, Checkers, Connect 4, Tic-Tac-Toe, Tower Defense, Parking Jam, Mini Golf

**Multiplayer** — Penguin Knockout, UNO, Imposter, Wavelength, Snake.io, Hole.io, Battleship, Pong, Tron, Space Race, Tanks, Pool, Dots & Boxes, Poker, Rock Paper Scissors

**Idle/Tycoon** — Idle Miner, Biz Tycoon, Slime Farmer

**Other** — Minecraft, Subway Surfers, Slope, Run 3, Drift Hunters, Fireboy & Watergirl, FNAF, Champion Island, Spelunky, Hextris, World's Hardest Game, and more

## Built With

HTML / CSS / JavaScript / Firebase / WebSockets / GitHub Pages

---

<div align="center">

**Copyright (c) 2025-2026 Mason P. All Rights Reserved.**

This source code may not be reproduced, distributed, modified, or used in any form without prior written permission from the copyright holder. Unauthorized copying or redistribution is strictly prohibited. The hosted website is provided for personal, non-commercial use only. Access to the website does not grant any rights to the underlying source code.

</div>
