  ...
  function buildCSS(t) {
    return `
      ...
      *:focus-visible { outline-color:${t.accent}!important; }
      ...
      .card.netflix-card { box-shadow:0 0 16px rgba(229,9,20,0.25)!important; }
      .card.netflix-card:hover { box-shadow:0 0 24px rgba(229,9,20,0.35)!important; }
      #announce-bar a { color:#e50914!important; }
...
(function() {
  ...
  function _open(manual) {
    // Force-clean any stale instances before creating a new one
    var stale = document.querySelectorAll('#' + ID);
  ...
  function _close() {
    var stale = document.querySelectorAll('#' + ID);
    for (var i = 0; i < stale.length; i++) stale[i].remove();
    if (_q && document.title === _t) document.title = _q;
    if (_qf) _setFav(_qf);
    _q = '';
    _qf = '';
    _m = false;
